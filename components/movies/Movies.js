...
export default function Movies() {
  ...
  return (
    <div>
      <h1 className=" text-white  lg:px-24 px-5 font-semibold py-2 text-left">
        Action Movies
      </h1>
      <div className="flex  justify-center">
        <div className=" scroll flex overflow-x-auto  md:scrollbar-default scrollbar-hide   space-x-8 w-[90%] lg:w-[85%]">
          {Shows.map((show, index) => (
            <div
              key={index}
              className=" flex flex-col items-center flex-shrink-0   h-auto w-auto "
            >
              <Image
                onClick={() => router.push(show.url, scroll)}
                src={show.image}
                alt={"action movie images"}
                loading="lazy"
                className=" h-[200px]  w-[200px] "
              />
              <h1
                onClick={() => router.push(show.url)}
                className=" cursor-default md:cursor-pointer hover:text-blue-500 text-white"
              >
                {show.title}
              </h1>
            </div>
